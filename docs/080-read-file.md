...
If the read buffer is full, then irrespective of whether we've hit EOF, we know that we need to process at least `Math.floor($READ_BUFFER_SIZE, 64)` message blocks; hence the following statement:
